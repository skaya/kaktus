...
function smarty_block_ajax_submit_form ($params, $content,  &$smarty, &$repeat)
{
  if ($content !== null)
  {
...
//  return 'SmartyAjax.update(\'' . $update_id . '\', \'' . $url .
  //  '\', \'' . $method . '\', \'' . $parameters . '\'); return false;';
    ...
    return '<form id="aj" name="aj" method="' . $method . '" action="' . $url .
      '" onsubmit="' . $onsubmit . ';SmartyAjax.datasubmit(\'' . $update_id . '\', \'' . $url .
    '\', this,\'' . $parameters .
      (isset($params['callback']) ? ', ' . $params['callback'] : '') . '\'); return false;"' .

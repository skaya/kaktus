...
function pic_edit() {
	...
	$kol = '';
	$image =$DB->select("select ?_pictures.id from ?_pictures left join ?_pictures_to_pages on ?_pictures.id=?_pictures_to_pages.obj_id where ?_pictures_to_pages.page_id=?",$_REQUEST['issue_id']);
	...
	//	$spaw->addPage(new SpawEditorPage("text_".$value,"Текст (".$value.")",stripslashes($pages["text_".$value])));
	//}
